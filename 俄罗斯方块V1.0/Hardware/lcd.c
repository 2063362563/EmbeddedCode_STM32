...
//断言宏定义
#define IS_CHN_FONT(font)   (((font) == CHN_font_12) || ((font) == CHN_font_24))
...
//显示一个汉字
void LCD_ShowSingleHZ(u16 x,u16 y,const u8 *pcStr,CHN_font_t font)
{  							  
    u8 temp,t1,t,i;
	u16 y0=y;
	if(IS_CHN_FONT(font)){
		if(font == CHN_font_12){
			//遍历寻找目标字模结构体
			for(i=0;i<sizeof(hz12_12)/sizeof(hz12_12[0]);i++){
				//判断结构体数组的汉字索引数组是否与要显示的中文字符一致，中文字符占两个字节，先判断第一个字符，再判断第二个字符
				if((hz12_12[i].Index[0] == *pcStr)&&(hz12_12[i].Index[1] == *(pcStr+1))){
					for(t=0;t<24;t++){   
						temp = hz12_12[i].Msk[t];
						for(t1=0;t1<8;t1++)
						{			   
							if(temp&0x80){
			...
			//遍历寻找目标字模结构体
			for(i=0;i<sizeof(hz24_24)/sizeof(hz24_24[0]);i++){
			
				//判断结构体数组的汉字索引数组是否与要显示的中文字符一致，中文字符占两个字节，先判断第一个字符，再判断第二个字符
				if((hz24_24[i].Index[0] == *pcStr)&&(hz24_24[i].Index[1] == *(pcStr+1))){
					for(t=0;t<72;t++){   
						temp = hz24_24[i].Msk[t];
						for(t1=0;t1<8;t1++)
						{			   
							if(temp&0x80){
								LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
//显示一个包含汉字和ASCII字符的字符串
void LCD_ShowHZ_ASCIIString(u16 x, u16 y, u8 *pcStr, CHN_font_t font)
{
    while(*pcStr != '\0') {
        if(*pcStr < 128) {
            // 显示一个ASCII字符
            LCD_ShowChar(x, y, *pcStr, font, 0);
            // 更新坐标，准备显示下一个字符
            x += (font/2);
            // 指向下一个字符
            pcStr++;
            ...
            // 显示一个汉字
            LCD_ShowSingleHZ(x, y, pcStr, font);
            // 更新坐标，准备显示下一个汉字
            if(font == CHN_font_12) {
                x += 12;
...
void Fill_LCD_Block(int x, int y, int color) {
    LCD_Fill(5 + 12 * x, 76 + 12 * y, 12 * x + 15, 86 + 12 * y, color);
}
